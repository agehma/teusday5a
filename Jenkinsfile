        ...
        stage('ContinuousDownload')
        {
            steps
            {
                script
                {
                    try
                    {
                        git 'https://github.com/agehma/teusday5a.git'
                    }
                    catch (Exception e1)
                    {
                        mail bcc: '', body: 'git failed to download the code', cc: '', from: '', replyTo: '', subject: 'git failed ', to: 'gt@gmail.com'
                        exit(1)
                    }
                }
            }
        ...
        stage('ContinuousDeployment')
        {
            steps
            {
                script
                {
                    ...
                    {
                        mail bcc: '', body: 'artifact not deployed to tomcat ', cc: '', from: '', replyTo: '', subject: 'dc failed ', to: 'mwt@gmail.com'
                        exit(1)
                    }
                }
            }
            ...
            steps
            {
                script
                {
                    try
                    {
                        ...
                        sh 'java -jar /var/lib/jenkins/workspace/DeclarativePipeline-Ex/testing.jar'
                    }
                    catch (Exception e4)
                    {
        ...
        stage('ContinuousDelivery')
        {
            steps
            {
                script
                {
                    ...
                    {
                        mail bcc: '', body: 'failed to deliver artifact into prodserver', cc: '', from: '', replyTo: '', subject: 'delivery failed ', to: 'dt@gmail.com'
                        exit(1)
                    }
                }
            }
